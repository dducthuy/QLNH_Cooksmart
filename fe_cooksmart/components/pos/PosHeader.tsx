...
export default function PosHeader({
  ...
  return (
    <header className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 shrink-0 shadow-sm z-10 w-full">
      <div className="flex items-center gap-4">
        <Link
          href="/admin"
          className="p-2 bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-600 transition-colors"
          title="Trở về quản trị"
        >
          <ArrowLeft size={18} />
        </Link>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <div
            className={`w-2 h-2 rounded-full ${
              hasActiveShift ? "bg-emerald-500 animate-pulse" : "bg-red-500"
            }`}
          ></div>
          ...
          className={`p-2.5 rounded-xl transition-all relative group ${
            totalNotiCount > 0
              ? pendingOrdersCount > 0
                ? "bg-violet-50 text-violet-600 animate-bounce-slow"
        ...
        >
          ...
          {totalNotiCount > 0 && (
            <span
              className={`absolute -top-1.5 -right-1.5 text-white text-[10px] font-black w-5 h-5 flex items-center justify-center rounded-full border-2 border-white shadow-sm ${
