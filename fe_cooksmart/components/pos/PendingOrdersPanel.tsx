...
export default function PendingOrdersPanel() {
    ...
    const [activeInvoices, setActiveInvoices] = useState<Record<string, HoaDon>>({}); // id_ban -> HoaDon (active)
    ...
    useEffect(() => {
        ...
        socket.on('don_qr_cho_duyet', handleNewOrder);
        return () => { socket.off('don_qr_cho_duyet', handleNewOrder); };
    }, [fetchPending, socket]);
    ...
    const handleApprove = async (hoaDonId: string) => {
        try {
            ...
            playNotificationSound(); // 🔔 Phát tiếng khi duyệt đơn thành công
            
            // Cập nhật lại màn hình POS chính nếu bàn đang xem là bàn vừa duyệt
            await refreshActiveOrder();
            
            const updated = pendingOrders.filter(o => o.id !== hoaDonId);
                    ...
                    <div className="relative w-full sm:w-[400px] bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                        ...
                        <div className="p-5 border-b border-gray-100 shrink-0">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => setIsPendingOrdersOpen(false)} className="p-2 text-gray-400 hover:bg-gray-100 rounded-xl transition-colors">
                                        <X size={20} />
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-100">
                                ...
                                >
                                    ...
                                    {pendingOrders.length > 0 && (
                                        <span className={`w-4 h-4 flex items-center justify-center rounded-full text-[8px] ${activeTab === 'orders' ? 'bg-[#d9a01e] text-white' : 'bg-gray-200 text-gray-500'
                                            }`}>
                                ...
                                >
                                    ...
                                    {notifications.length > 0 && (
                                        <span className={`w-4 h-4 flex items-center justify-center rounded-full text-[8px] ${activeTab === 'notifications' ? 'bg-emerald-500 text-white' : 'bg-gray-200 text-gray-500'
                                            }`}>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 pos-scrollbar space-y-4">
                            {activeTab === 'notifications' ? (
                                <>
                                    <div className="flex items-center justify-between px-1">
                                        <div className="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-2">
                                            ...
                                            Món đã hoàn thành ({notifications.filter(n => n.type === 'DISH_DONE').length})
                                        </div>
                                    ...
                                    {notifications.filter(n => n.type === 'DISH_DONE').length === 0 ? (
                                        <div className="h-60 flex flex-col items-center justify-center gap-3 text-center grayscale opacity-40">
                                            ...
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Chưa có món nào xong</p>
                                        </div>
                                    ) : (
                                        Object.entries(
                                            notifications
                                                .filter(n => n.type === 'DISH_DONE')
                                                .reduce((acc: any, noti) => {
                                            ...
                                            <div key={hdId} className="bg-emerald-50/40 border border-emerald-100 rounded-2xl overflow-hidden shadow-sm shadow-emerald-100/50">
                                                <div className="bg-emerald-500 px-3 py-1.5 flex justify-between items-center">
                                                    <p className="text-[10px] font-black text-white uppercase tracking-tighter flex items-center gap-2">
                                                        <UtensilsCrossed size={12} />
                                                        {group.so_ban ? `BÀN ${group.so_ban}` : 'KHÔNG XÁC ĐỊNH'}
                                                    </p>
                                                    <button
                                                        onClick={() => group.items.forEach((item: any) => removeNotification(item.id))}
                                                        className="text-[10px] font-bold opacity-80 hover:opacity-100 text-white transition-colors"
                                                    >
                                                        Xóa hết
                                                    </button>
                                                </div>
                                                <div className="p-2 space-y-1">
                                                    {group.items.map((noti: any) => (
                                                        <div key={noti.id} className="group relative bg-white border border-emerald-50 rounded-xl p-2.5 flex items-center gap-3 hover:border-emerald-200 transition-all">
                                                            <div className="w-1.5 h-1.5 rounded-full bg-emerald-400"></div>
                                                            <div className="flex-1">
                                                                <p className="text-xs font-bold text-gray-800 leading-tight">
                                                                    {noti.data?.ten_mon || 'Món ăn'}
                                                                </p>
                                                                <p className="text-[10px] text-gray-600 mt-0.5">Đã hoàn thành, sẵn sàng phục vụ</p>
                                                                <p className="text-[8px] text-gray-400 font-medium mt-1 uppercase tracking-tighter">{timeAgo(noti.time)}</p>
                                                            </div>
                                                            <button
                                                                onClick={() => removeNotification(noti.id)}
                                                                className="p-1 text-gray-300 hover:text-red-400 hover:bg-red-50 rounded-lg transition-all opacity-0 group-hover:opacity-100"
                                                            >
                                                                <X size={12} />
                                                            </button>
                                                        </div>
                                ...
                                <>
                                    <h3 className="text-[10px] font-black text-[#d9a01e] uppercase tracking-widest px-1">Đơn hàng cần duyệt ({pendingOrders.length})</h3>
                                    {isLoading ? (
                                        ...
                                        <div className="h-60 flex flex-col items-center justify-center gap-3 text-center grayscale opacity-40">
                                            ...
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Không có đơn chờ</p>
                                        </div>
                                    ) : (
                                        pendingOrders.map(order => (
                                            <div key={order.id} className="bg-white rounded-2xl border border-amber-100 shadow-sm overflow-hidden ring-1 ring-[#d9a01e]/10">
                                                {/* Top strip */}
                                                ...
                                                <div className="p-3.5">
                                                    ...
                                                    <div className="flex items-start justify-between mb-3">
                                                        <div>
                                                            <p className="font-black text-gray-800 text-base leading-none">
                                                                {order.BanAn?.so_ban ? `BÀN ${order.BanAn.so_ban}` : 'Bàn không xác định'}
                                                            </p>
                                                    ...
                                                    {order.id_ban && activeInvoices[order.id_ban] && (
                                                        <div className="mb-3">
                                                            ...
                                                            <div className="bg-amber-50/50 border border-amber-100 rounded-xl p-2 space-y-1">
                                                                {activeInvoices[order.id_ban].ChiTietHoaDons?.map((ct: any, i: number) => (
                                                                    <div key={i} className="flex items-center gap-2 text-[11px]">
                                                                        <span className="text-amber-800/60 font-medium flex-1 truncate">
                                                                            {ct.MonAn?.ten_mon || ct.Combo?.ten_combo || 'Món ăn'}
                                                                        </span>
                                                                        <span className="font-bold text-amber-700">x{ct.so_luong}</span>
                                                                    </div>
                                                                ))}
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div className="flex justify-between items-center mb-4">
                                                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tổng tiền đơn này</span>
