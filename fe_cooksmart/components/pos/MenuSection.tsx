      ...
      <div className="aspect-square bg-gray-50 rounded-2xl mb-3 overflow-hidden relative">
        <img
          src={
            ...
            `https://ui-avatars.com/api/?name=${encodeURIComponent(combo.ten_combo)}&background=d9a01e&color=fff&size=100`
          }
          alt={combo.ten_combo}
          className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
        />
        <div className="absolute top-2 right-2 bg-[#d9a01e] text-white text-[8px] font-black uppercase px-2 py-1 rounded-lg">
          Combo
        </div>
      </div>
      <h3 className="font-bold text-gray-800 text-sm leading-tight px-1 line-clamp-2">
        {combo.ten_combo}
      </h3>
...
const DishCard = ({
  ...
  item: MonAn;
  onAddToCart: (item: MonAn) => void;
}) => {
  return (
    <div
      onClick={() => item.con_hang && onAddToCart(item)}
    ...
    >
      <div className="aspect-square bg-gray-50 rounded-2xl mb-3 overflow-hidden relative">
        <img
          src={
            item.hinh_anh_mon ||
            `https://ui-avatars.com/api/?name=${encodeURIComponent(item.ten_mon)}&background=random&color=fff&size=100`
          }
          alt={item.ten_mon}
          className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
        />
        {!item.con_hang && (
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center backdrop-blur-[1px]">
            <span className="bg-black/80 text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full shadow-lg">
              Hết Hàng
            </span>
          </div>
        )}
      ...
      <div className="mt-auto pt-3 px-1 flex items-center justify-between">
        <span
          className={`font-black tracking-tight ${!item.con_hang ? "text-gray-400" : "text-[#d9a01e]"}`}
        >
          {new Intl.NumberFormat("vi-VN", {
            style: "currency",
            currency: "VND",
...
export default function MenuSection({
  ...
  const categoryNames = [
    "Tất Cả",
    "Combo",
    ...categories.map((c) => c.ten_danh_muc),
  ];
  ...
  const filteredDishes = items.filter(
    (item) =>
      item.con_hang &&
      (activeTab === "Tất Cả" ||
        (activeTab !== "Combo" && item.DanhMuc?.ten_danh_muc === activeTab)) &&
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase font-bold text-gray-400 mt-0.5">
              {items.length + combos.length} Món ăn & Combo
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
            ...
            {filteredCombos.map((combo) => (
              <ComboCard
                key={combo.id}
                combo={combo}
                onViewCombo={setViewingCombo}
                onAddToCart={(c) =>
                  addToCart({
                    id_combo: c.id,
                    ten_mon: c.ten_combo,
                    gia_tien: c.gia_tien,
                    hinh_anh_mon: c.hinh_anh_combo,
                    so_luong: 1,
                  })
                }
              />
