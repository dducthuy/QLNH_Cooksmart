...
interface OrderCartProps {
  ...
  setViewingCombo: (combo: any) => void;
}
...
export default function OrderCart({
  ...
  const handlePrint = () => {
    ...
    // Group items for printing (ignore status, group by id + note)
    const printMap = new Map<string, any>();
    (activeOrder.ChiTietHoaDons || []).forEach((item: any) => {
      const key = `${item.id_mon_an || item.id_combo}-${item.ghi_chu || ""}`;
      if (printMap.has(key)) {
        printMap.get(key).so_luong += item.so_luong;
      } else {
        printMap.set(key, { ...item });
      }
    });
    const printItems = Array.from(printMap.values());

    const itemsHtml = printItems
      .map((item: any) => `
        <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
          <span>${item.so_luong}x ${item.MonAn?.ten_mon || item.Combo?.ten_combo || "Món ăn"}</span>
          <span>${new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(
        (item.MonAn?.gia_tien || item.Combo?.gia_tien || 0) * item.so_luong
      )}</span>
        </div>
      `)
      .join("");
    ...
    const html = `
      ...
      <head>
        <style>
          ...
          .header p { margin: 0 0 3px 0; font-size: 14px; }
          .divider { border-top: 1px dashed #000; margin: 15px 0; }
          .item { display: flex; justify-content: space-between; margin-bottom: 5px; font-size: 14px; }
          .total { font-weight: bold; font-size: 16px; margin-top: 15px; border-top: 1px solid #000; padding-top: 10px; display: flex; justify-content: space-between; }
          @media print {
            body { padding: 0; }
      ...
      <body>
        <div class="receipt">
          <div class="header">
            <h2>COOKSMART</h2>
            <p>Bàn: ${selectedTable?.so_ban || ""}</p>
            <p>Hóa đơn: #${activeOrder.id?.substring(0, 8) || ""}</p>
            <p>Thời gian: ${new Date().toLocaleString("vi-VN")}</p>
          </div>
          <div class="divider"></div>
          ${itemsHtml}
          <div class="divider"></div>
          ${activeOrder.giam_gia > 0
        ? `
            <div class="item">
              <span>Giảm giá:</span>
        ...
        : ""
          ...
          <div class="total">
            <span>Tổng tiền:</span>
            <span>${new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(total - (activeOrder.giam_gia || 0))}</span>
          </div>
          <div class="divider"></div>
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "DangCho":
        return (
          <span className="px-2 py-1 rounded-md bg-amber-50 text-amber-600 border border-amber-100 text-[10px] font-black uppercase tracking-tight">
            Đang chờ
          </span>
        );
      case "DangNau":
        return (
          <span className="px-2 py-1 rounded-md bg-amber-50 text-[#d9a01e] border border-amber-100 text-[10px] font-black uppercase tracking-tight animate-pulse">
            Đang nấu
          </span>
        );
      case "DaXong":
        return (
          <span className="px-2 py-1 rounded-md bg-emerald-50 text-emerald-600 border border-emerald-100 text-[10px] font-black uppercase tracking-tight">
            Đã xong
          </span>
        );
      case "DaLayDi":
        return (
          <span className="px-2 py-1 rounded-md bg-blue-50 text-blue-600 border border-blue-100 text-[10px] font-black uppercase tracking-tight">
            Đã phục vụ
          </span>
        );
      default:
                  ...
                  {(() => {
                    ...
                    return displayItems.map((item, index) => (
                      <div
                        ...
                        className="p-3 bg-gray-50/50 rounded-2xl border border-gray-100 flex flex-col gap-1.5 opacity-80 border-dashed"
                      >
                        ...
                        <div className="flex justify-between items-center">
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-black text-gray-800 bg-white px-2 py-0.5 rounded-md border border-gray-100">
                              x{item.so_luong}
                            </span>
                            {item.ghi_chu && (
                          ...
                          <p className="text-gray-500 font-bold text-xs">
                            {new Intl.NumberFormat("vi-VN", {
                              style: "currency",
                              currency: "VND",
                            }).format((item.MonAn?.gia_tien || item.Combo?.gia_tien || 0) * item.so_luong)}
                          </p>
                        </div>
                      </div>
                    ));
            ...
            {cart.length > 0 && (
              <div>
                ...
                <div className="space-y-3">
                  {cart.map((item, index) => (
                    <div
                      key={`cart-${item.id_mon_an}-${index}`}
                      className="p-3 bg-amber-50/30 rounded-2xl border border-amber-100/50 flex flex-col gap-2 group shadow-sm ring-1 ring-[#d9a01e]/5"
                    >
                      <div className="flex justify-between items-start gap-2">
                        <h4
                      ...
                      <div className="flex justify-between items-center mt-1 pt-2 border-t border-amber-100 border-dashed">
                        ...
                        <div className="flex items-center gap-3 bg-white rounded-xl p-1 border border-gray-200 shadow-sm">
                          ...
                          <span className="text-sm font-black w-4 text-center text-gray-800">
                            {item.so_luong}
                          </span>
                          <button
                            onClick={() => updateQuantity(item.id_mon_an || item.id_combo!, 1, !!item.id_combo)}
      ...
      <div className="p-5 border-t border-gray-100 shrink-0 bg-gray-50 rounded-b-3xl">
          ...
          <div className="flex justify-between items-center pt-3 border-t border-gray-100 border-dashed">
            ...
            <span className="text-2xl font-black text-[#d9a01e]">
              {new Intl.NumberFormat("vi-VN", {
                style: "currency",
                currency: "VND",
              }).format(total - (activeOrder?.giam_gia || 0))}
            </span>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          {activeOrder && activeOrder.ChiTietHoaDons && activeOrder.ChiTietHoaDons.length > 0 && (
            <button
              onClick={handlePrint}
              className="px-4 py-4 rounded-2xl font-black uppercase text-xs tracking-widest bg-gray-100 hover:bg-gray-200 text-gray-700 shadow-sm active:scale-95 transition-all flex items-center justify-center gap-2"
              title="In hóa đơn"
            >
              <Printer size={18} />
          ...
          {(() => {
            ...
            const allDone = hasItems && items.every((i: any) => ["DaXong", "DaLayDi"].includes(i.trang_thai_mon));
