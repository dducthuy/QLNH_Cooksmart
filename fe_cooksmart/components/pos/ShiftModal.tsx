...
export default function ShiftModal({
  ...
  const canCloseShift =
    isAdmin ||
    (currentShiftData && currentShiftData.ca_lam_viec.id_nhan_vien === userId);
  ...
  const handleSubmitClose = async () => {
    ...
    try {
      setIsSubmitting(true);
      await ketCaService.chotCa(currentShiftData.ca_lam_viec.id, {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
      ...
      {mode === "OPEN" && (
        <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl p-8 flex flex-col items-center animate-in fade-in zoom-in-95 duration-300">
          ...
          <h2 className="text-2xl font-black text-gray-800 uppercase tracking-widest text-center">
            Bắt Đầu Phiên Mới
          </h2>
          <p className="text-sm font-medium text-gray-500 mt-2 text-center mb-8">
            Vui lòng nhập số tiền lẻ ban đầu có trong{" "}
            <b className="text-gray-700">Két Tiền Mặt</b> của bạn để bắt đầu làm việc.
          </p>
      ...
      {/* 2. KHỐI MODE = CLOSE (Báo cáo & Chốt ca) */}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-white font-black text-lg uppercase tracking-widest">
                  Báo Cáo Tạm Tính & Chốt Ca
                </h2>
                <p className="text-gray-400 text-xs font-mono mt-1">
                  Nhân viên:{" "}
                  <span className="text-amber-400 font-bold">
                    {currentShiftData.ca_lam_viec.NguoiDung?.ho_ten ||
                      currentShiftData.ca_lam_viec.NguoiDung?.ten_dang_nhap ||
                      "N/A"}
                  </span>
                  <span className="mx-2">|</span>
                  Mở lúc:{" "}
                  {new Date(currentShiftData.ca_lam_viec.thoi_gian_bat_dau).toLocaleString("vi-VN")}
                </p>
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-[50%] p-8 bg-gray-50 border-r border-gray-100 flex flex-col gap-6">
              <div className="p-5 bg-white rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
                ...
                <span className="text-xl font-black text-gray-800">
                  {formatVND(Number(currentShiftData.ca_lam_viec.tien_dau_ca))}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-white rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <span className="text-lg font-black text-emerald-600">
                    +{formatVND(currentShiftData.doanh_thu_tien_mat)}
                  </span>
                </div>
                <div className="p-4 bg-white rounded-2xl border border-gray-100 shadow-sm">
                  <span className="block text-xs font-bold text-gray-400 uppercase mb-2">Chuyển Khoản</span>
                  <span className="text-lg font-black text-blue-600">
                    +{formatVND(currentShiftData.doanh_thu_chuyen_khoan)}
                  </span>
                </div>
              </div>
              ...
              <div className="p-4 bg-white rounded-2xl border border-gray-100 shadow-sm">
                ...
                <span className="text-lg font-black text-gray-800">
                  {currentShiftData.tong_so_don} đơn thành công
                </span>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                <div className="px-4 py-3 bg-red-50 border-b border-red-100 flex justify-between items-center">
                  ...
                  <span className="text-sm font-black text-red-600">
                    -{formatVND(currentShiftData.tong_tien_chi)}
                  </span>
                </div>
              ...
              <div className="mt-auto pt-6 border-t border-gray-200">
                ...
                <div className="flex flex-col gap-0.5 mt-2 opacity-60">
                  <p className="text-[10px] font-medium text-gray-500">
                    Công thức: [Đầu ca] + [Doanh thu mặt] - [Chi tiêu]
                  </p>
                  <p className="text-[10px] text-gray-400 italic">
                    ({formatVND(Number(currentShiftData.ca_lam_viec.tien_dau_ca))} + {formatVND(currentShiftData.doanh_thu_tien_mat)} - {formatVND(currentShiftData.tong_tien_chi)})
                  </p>
                </div>
              </div>
            </div>

            {/* Cột phải: Form bàn giao két và chi tiêu vặt */}
            <div className="flex-1 p-8 bg-white flex flex-col">
              ...
              <label className="block text-xs font-black uppercase text-gray-400 tracking-widest mb-3">
                Nhập Số Tiền Có Thực Tế Trong Két Đang Đếm Được
              </label>
              ...
              {/* Khối hiển thị chênh lệch */}
              {cashInput && (
                <div
                  className={`mt-6 p-5 rounded-2xl border-2 flex items-center gap-4 ${parsedCash === currentShiftData.tien_mat_ly_thuyet
                ...
                >
                  {parsedCash === currentShiftData.tien_mat_ly_thuyet ? (
                    ...
                    <AlertTriangle
                      size={32}
                      className={`${parsedCash < currentShiftData.tien_mat_ly_thuyet ? "text-red-500" : "text-amber-500"} flex-shrink-0`}
                    />
                  )}
              ...
              {/* Nút hành động chính dựa trên quyền đóng ca */}
              {!canCloseShift ? (
