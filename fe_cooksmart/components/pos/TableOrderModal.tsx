  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      ...
      try {
        ...
        let active = invoices.find(
          (i) =>
            i.trang_thai_hd === "DangPhucVu" || i.trang_thai_hd === "ChoXuLy",
        );
  ...
  const handleFetchOtherTables = async (targetMode: "transfer" | "merge") => {
    try {
      ...
      const filtered = data.filter((b: any) => {
        ...
        if (targetMode === "merge")
          return (
            b.trang_thai_ban === "DangPhucVu" ||
            b.trang_thai_ban === "ChoThanhToan"
          );
        return true;
      });
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                ></span>
                <p className="text-[11px] uppercase font-bold text-gray-500 tracking-wider w-[100px]">
                  {table.status}
                </p>
              </div>
            </div>
          ...
          {!hasOrder ? (
            <div className="py-8 flex flex-col items-center justify-center text-center gap-3">
              ...
              <p className="text-xs text-gray-400 max-w-xs">
                Nhấn vào "Mở Bàn" bên dưới để bắt đầu order món cho khách.
              </p>
            </div>
          ) : loading ? (
            ...
            <div className="py-12 flex flex-col items-center justify-center text-center gap-3">
              <p className="text-sm font-bold text-gray-500">
                Bàn này chưa có hóa đơn hệ thống
              </p>
              <p className="text-xs text-gray-400">
                Có thể khách chưa bắt đầu gọi món.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              <div className="flex items-center justify-between text-xs font-bold text-gray-400 uppercase tracking-widest pb-2 mb-2 border-b border-gray-200">
                ...
                <span className="flex items-center gap-1">
                  <Clock size={12} />{" "}
                  {new Date(invoice.thoi_gian_tao).toLocaleTimeString("vi-VN", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </span>
              ...
              {orderItems.map((item) => (
                <div
                  key={item.id}
                  className="bg-white p-3.5 rounded-2xl border border-gray-100 flex flex-col gap-2 shadow-sm"
                >
